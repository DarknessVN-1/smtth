...
local function DoSkills()
    ...
    -- Hàm tìm tất cả skill (Tool có spellEvent)
    local function getAllSpells()
        local spells = {}
        local function collect(container)
            for _, tool in ipairs(container:GetChildren()) do
                if tool:IsA("Tool") then
                    local remote = tool:FindFirstChild("spellEvent")
                    if remote and remote:IsA("RemoteEvent") then
                        table.insert(spells, tool)
                    end
                end
            end
        end
    ...
    -- Spam liên tục
    local INTERVAL = 0.03 -- 30ms
    task.spawn(function()
        while true do
            task.wait(INTERVAL)
            local spells = getAllSpells()
            for _, tool in ipairs(spells) do
                local remote = tool:FindFirstChild("spellEvent")
                if remote then
                    remote:FireServer()
                end
