...
local function getE()
    ...
    for _,v in pairs(workspace.dungeon:GetChildren())do if v:FindFirstChild("enemyFolder")then return v.enemyFolder:GetChildren()end end
    return{}
end

local function getC()
    local e,d=nil,1/0
    for _,v in pairs(getE())do local hr=v:FindFirstChild("HumanoidRootPart")local head=v:FindFirstChild("Head")
    ...
    e.DescendantAdded:Connect(function(o)
        if o:IsA("BasePart") and o.Size.Magnitude>6 then
            ...
            if dist<radius then
                ...
                h:MoveTo(o.Position+dir*(radius+12))
...
local function loop()
    while task.wait(.2)do
        ...
        if e and e:FindFirstChild("HumanoidRootPart")then
            watch(e)
            if d<12 then h:MoveTo(hrp.Position+(hrp.Position-e.HumanoidRootPart.Position).Unit*15)log("[RETREAT]"..e.Name,Color3.fromRGB(255,200,0))
            elseif d>20 then h:MoveTo(e.HumanoidRootPart.Position)log("[APPROACH]"..e.Name,Color3.fromRGB(0,200,255))
            else h:MoveTo(hrp.Position)log("[HOLD]"..e.Name,Color3.fromRGB(0,255,0))end
        end
    end
end
